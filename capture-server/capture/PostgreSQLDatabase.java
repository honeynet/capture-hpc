...
public class PostgreSQLDatabase extends Database {
	...
	public boolean setStatus(String urlid, String status, String visitStartTime, String visitFinishTime, String clientProgram)
	{
		...
		try
		{
			...
			stmt.executeUpdate("UPDATE url_operation SET visitstarttime=to_timestamp(\'" +visitStartTime+"\','DD/MM/YYYY HH24:MI:SS.MS\'), " +
				                                                                "visitfinishtime=to_timestamp(\'" +visitFinishTime+"\','DD/MM/YYYY HH24:MI:SS.MS\'), " +
				                                                                "clientprogram_id="+clientProgramid+", "+
				                                                                "status_id=\'" + statusid +
				                           "\' WHERE url_id="+urlid+" AND operation_id="+operationid);
			stmt.executeUpdate("UPDATE url SET lastvisittime=to_timestamp(\'" +visitFinishTime+"\','DD/MM/YYYY HH24:MI:SS.MS\'), " +
	...
	public boolean finishOperation()
	{
		...
		try
		{
			...
			if (operationid!=null) {
				...
				stmt.executeUpdate("UPDATE operation SET visitfinishtime=to_timestamp(\'" +date+"\','DD/MM/YYYY HH24:MI:SS.MS\') "+
	...
	public void loadInputUrlFromFile(final String inputUrlsFile) 
	{
		...
		try
		{
			...
			} else 
			{
				...
				rs=stmt.executeQuery("INSERT INTO honeypot(honeypot_id, ipaddress, port) Values (DEFAULT, \'"+serverip+"\', "+serverport+")"+" RETURNING honeypot_id");
				if (rs.next())
				{
					honeypotid=rs.getString(1);
				} 
			...
			//add new operation
			rs=stmt.executeQuery("INSERT INTO operation(operation_id, description, honeypot_id) Values (DEFAULT, \'"+inputUrlsFile+"\', \'"+honeypotid+"\')"+" RETURNING operation_id");
			if (rs.next())
			{
			...
			//update visit start time for new operation
			SimpleDateFormat sf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss.S");
			String date = sf.format(new Date());
			stmt.executeUpdate("UPDATE operation SET visitstarttime=to_timestamp(\'" +date+"\','DD/MM/YYYY HH24:MI:SS.MS\') "+
   ...
   //load all urls from url table and then run operation
	public void loadInputUrlFromDatabase()
	{
		...
		try
		{
			...
			{
				//insert a new honeypot 
				rs=stmt1.executeQuery("INSERT INTO honeypot(honeypot_id, ipaddress, port) Values (DEFAULT, \'"+serverip+"\', "+serverport+")"+" RETURNING honeypot_id");
				if (rs.next())
				{
					honeypotid=rs.getString(1);
				} 
			...
			setSystemStatus(true);
			//add new operation
			rs=stmt1.executeQuery("INSERT INTO operation(operation_id, description, honeypot_id) Values (DEFAULT, \'"+"Load urls from database"+"\', \'"+honeypotid+"\')"+" RETURNING operation_id");
			if (rs.next())
			{
				operationid=rs.getString(1); setCurrentOperation(operationid);
			} 
			...
			SimpleDateFormat sf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss.S");
			String date = sf.format(new Date());
			stmt1.executeUpdate("UPDATE operation SET visitstarttime=to_timestamp(\'" +date+"\','DD/MM/YYYY HH24:MI:SS.MS\') "+
												"WHERE operation_id="+operationid+" AND visitstarttime IS NULL"); 
	...
	public void storeEvent(String urlid, String event)
	{
		...
		try
		{
			...
			PreparedStatement ps = con.prepareStatement("INSERT INTO event(url_id, time, operation_id,  type,  process, action, object1, object2) "+
				" VALUES ("+urlid+", to_timestamp(\'" +str[1]+"\','DD/MM/YYYY HH24:MI:SS.MS\'), "+getCurrentOperation()+", ?, ?, ?, ?, ?)");
