...
-----------------------------------------------------------------
Download the Capture client component and extract it into the directory C:\ so the files are located in C:\. Open Windows Explorer and navigate to this directory. Right click on the inf files and select install. When installing the fltmgr.inf, you might be prompted for the location of additional files. These are located in the i386 directory on your Windows XP installation CD.
...
-------------------------
In addition to the exclusion list, one can configure the client identifiers of the client honeypot in the clients.conf file. The client identifiers are pointers to client applications that can be used to interact with a server, such as Internet Explorer, Firefox, Opera, but also other applications that can retrieve content from a network, such as Microsoft Word, Adobe Acrobat Reader. Simply add the client identfier and fully qualifying path name to the client.conf file and specify the client identifier with the url that should be interacted with in the uri list that is piped to the server, e.g. Firefox::http://www.google.com.

Since some applications (like Adobe Acrobat) are unable to retrieve content files directly, Capture might retrieve the files, save them to a temporary folder, and then open the file with the application. To enable such behavior, edit the clients.conf file and specify "yes" at the end of the line.
